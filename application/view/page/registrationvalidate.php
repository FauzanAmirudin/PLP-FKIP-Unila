    ...
    <div class="card-header flex-header">
          ...
          <table class="modern-table" id="validate-table">
            ...
            foreach ($data as $key => $r) {
              ...
              <tr class="validate-row" data-index="<?= $key ?>">
                ...
                <td class="font-semibold"><?= htmlspecialchars($r["NAMA"]) ?></td>
                <td class="text-slate-600"><?= htmlspecialchars($r["NPM"]) ?></td>
                <td class="text-slate-600"><?= htmlspecialchars($r["PROGRAMSTUDI"]) ?></td>
                <td class="text-slate-500"><?= htmlspecialchars($r["JENISKELAMIN"]) ?></td>
                <td class="text-slate-500"><?= htmlspecialchars($r["NOTELEPON"]) ?></td>
                <td class="text-center"><span class="badge <?= $badgeClass ?>"><?= $statusTeks ?></span></td>
                <td class="text-center"><button class="btn-action-view" onclick="validate(<?= $berkasId ?>,'<?= $namaSafe ?>','<?= $r['NPM'] ?>','<?= $berkasSafe ?>')">Action</button></td>
              </tr>
              <?php
              $n++;
            }
      ...
      <?php
        ...
        echo '<div class="empty-state">
          ...
          <p>Tidak ada peserta yang sesuai dengan filter yang dipilih.</p>
        </div>';
      }
      ?>
  </div>
...
<div id="modal" class="modal-overlay">
  <div class="modal-modern-card">
      ...
      <div class="modal-modern-body">
          ...
          <form action="" method="post" enctype="multipart/form-data" id="res-lap-form" class="validator-form">
            <input type="hidden" id="data-ID" name="idmahasiswa" value="">
            <input type="hidden" id="data-NPM" name="npmmahasiswa" value="">
            
            <div class="form-group-modern">
              <label for="data-NOTE">Catatan Validator</label>
              <textarea id="data-NOTE" name="catatanberkas" class="input-control" placeholder="Tuliskan alasan penolakan atau instruksi perbaikan..."></textarea>
            </div>
...
<script type="text/javascript">
  function validate(id, nama, npm, berkas) {
    ...
    let linkBerkas = (berkas !== '') ? "<a href='" + berkas + "' target='_blank' class='btn-download' title='Klik untuk mendownload file ZIP'>⬇️ Download File .zip</a>" : "<span class='status-error'>❌ Belum Upload Berkas</span>";
    document.getElementById('dta-mhs').innerHTML = "<div class='info-group'><span>Nama Lengkap</span><strong>" + nama + "</strong></div><div class='info-group'><span>NPM</span><strong>" + npm + "</strong></div><div class='info-group'><span>Berkas Pendaftaran</span>" + linkBerkas + "</div>";
  ...
  function ajaxPOST(form, button, type) {
    ...
    aj_data.addValue("status=" + type).setCallback(function(text, element) {
      let relodBtn = '<button class="btn-action-view btn-reload" onClick="location.reload()">Perbaharui Daftar</button>';
      element.innerHTML = '<div class="ajax-notif"><b>' + text + '</b><br>' + relodBtn + '</div>';
