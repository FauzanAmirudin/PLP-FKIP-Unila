...
// Fallback cerdas: Jika tahun di pengaturan web (CURENTYEAR) tidak/belum memiliki pendaftar,
// secara otomatis gunakan tahun pendaftaran mahasiswa terbaru yang ada di database.
$checkData = $dbAccess->reset()->tabel('databerkas')->where(["TAHUNDAFTAR" => $curentYear])->result_array();
if (empty($checkData)) {
    $latestReg = $dbAccess->reset()->tabel('databerkas')->order('TAHUNDAFTAR', 'DESC')->limit(1)->result_fetch_array();
    if (!empty($latestReg) && !empty($latestReg['TAHUNDAFTAR'])) {
        $curentYear = $latestReg['TAHUNDAFTAR'];
    }
}
...
$seriesArr = [];
foreach ($data as $d) {
  if (empty($d["PROGRAMSTUDI"]) || $d["JUMLAHPESERTA"] == 0) continue;
  $labelsArr[] = "'" . addslashes($d["PROGRAMSTUDI"]) . "'";
  $seriesArr[] = (int)$d["JUMLAHPESERTA"];
}
$label = implode(", ", $labelsArr);
$series = implode(", ", $seriesArr);

$dataSex = $dbAccess->reset()->join('databerkas', 'datamahasiswa.USRKEY = databerkas.USRKEY')->column("JENISKELAMIN, COUNT(NPM) AS JUMLAHPESERTA", FALSE)->where(["TAHUNDAFTAR" => $curentYear])->group("JENISKELAMIN")->result_array('datamahasiswa');

$jnsklLabelsArr = [];
$jnsklSeriesArr = [];
foreach ($dataSex as $d) {
  if ($d["JUMLAHPESERTA"] == 0) continue;
  $gender = !empty($d["JENISKELAMIN"]) ? $d["JENISKELAMIN"] : 'Data Belum Lengkap';
  $jnsklLabelsArr[] = "'" . addslashes($gender) . "'";
  $jnsklSeriesArr[] = (int)$d["JUMLAHPESERTA"];
}
$jnskllabel = implode(", ", $jnsklLabelsArr);
$jnsklseries = implode(", ", $jnsklSeriesArr);

$dataVerifRaw = $dbAccess->reset()->join('databerkas', 'datamahasiswa.USRKEY = databerkas.USRKEY')->column("( SELECT STATUSBERKAS FROM datastatus WHERE datastatus.USRKEY = datamahasiswa.USRKEY ORDER BY id DESC LIMIT 1 ) AS STATUSBERKAS ", FALSE)->where(["TAHUNDAFTAR" => $curentYear])->result_array('datamahasiswa');
$dataVerifiksi = [];
foreach ($dataVerifRaw as $d) {
  if (empty($d["STATUSBERKAS"])) $d["STATUSBERKAS"] = 'Pengajuan';
...
foreach ($dataVerifiksi as $n => $d) {
  if ($d == 0) continue;
  $vrfLabelsArr[] = "'" . addslashes($n) . "'";
  $vrfSeriesArr[] = (int)$d;
}
$vrflabel = implode(", ", $vrfLabelsArr);
$vrfseries = implode(", ", $vrfSeriesArr);
?>
...
<script language="javascript" type="text/javascript">
  ...
  if (data.series.length > 0) {
    ...
    var pieOptions = {
      labelInterpolationFnc: function(label, index) {
        ...
        let val = data.series[index] || 0;
        let num = sum > 0 ? Math.round((val / sum) * 100) + '%' : '0%';
        return num;
      }
    };
    ...
    new Chartist.Pie('#presentasePendaftar', data, pieOptions, responsiveOptions);
  } else {
    var pEl = document.getElementById('dataPendaftar');
    if (pEl) pEl.innerHTML = '<div style="text-align:center; padding: 40px; color: #94a3b8;">Belum ada data pendaftar untuk tahun ini</div>';
    var prEl = document.getElementById('presentasePendaftar');
    if (prEl) prEl.innerHTML = '<div style="text-align:center; padding: 40px; color: #94a3b8;">Belum ada data pendaftar untuk tahun ini</div>';
  }
  ...
  if (data2.series.length > 0) {
    var options2 = {
      labelInterpolationFnc: function(label, index) {
        let sum = data2.series.reduce(function(total, value) {
          return total + value
        }, 0);
        let val = data2.series[index] || 0;
        let num = sum > 0 ? Math.round((val / sum) * 100) + '%' : '0%';
    ...
    var vEl = document.getElementById('dataTerverifikasi');
    if (vEl) vEl.innerHTML = '<div style="text-align:center; padding: 40px; color: #94a3b8;">Belum ada data verifikasi berkas</div>';
  }
  ...
  if (data3.series.length > 0) {
    var options3 = {
      labelInterpolationFnc: function(label, index) {
        let sum = data3.series.reduce(function(total, value) {
          return total + value
        }, 0);
        let val = data3.series[index] || 0;
        let num = sum > 0 ? Math.round((val / sum) * 100) + '%' : '0%';
        return label + ' - ' + num + " (" + val + ")";
      }
    };
    ...
    var jEl = document.getElementById('dataPeserta');
    if (jEl) jEl.innerHTML = '<div style="text-align:center; padding: 40px; color: #94a3b8;">Belum ada data jenis kelamin</div>';
  }
</script>
