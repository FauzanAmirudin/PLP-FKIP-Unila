    ...
    <div class="dashboard-card">
        ...
        <div class="schedule-container">
          <div class="schedule-card">
            ...
            if (!empty($jadwal_list)) { 
                foreach(['PLP1', 'PLP2'] as $jenis) {
                    if(!empty($data_plp[$jenis])) {
            ?>
                <h3 style="font-family: 'Poppins', sans-serif; font-size: 16px; margin-top: 15px; margin-bottom: 10px; color: #1e293b;">Jadwal <?php echo ($jenis == 'PLP1') ? 'PLP 1' : 'PLP 2'; ?></h3>
                <div class="table-responsive" style="margin-bottom: 20px;">
                  <table class="modern-table schedule-table">
                    <thead>
                      <tr>
                        <th class="col-kegiatan" style="text-align: center; width: 100px;">Minggu Ke-</th>
                        <th class="col-deskripsi">Rincian Kegiatan</th>
                        <th class="col-mulai" style="text-align: center;">Kegiatan Ke-</th>
                        <th class="col-selesai" style="text-align: center;">JP/Hari</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php foreach ($data_plp[$jenis] as $jadwal) { ?>
                        <tr>
                          <td style="text-align: center; font-weight: bold;"><?= htmlspecialchars($jadwal['MINGGU_KE']) ?></td>
                          <td class="td-kegiatan"><?= nl2br(htmlspecialchars($jadwal['JENISKEGIATAN'])) ?></td>
                          <td style="text-align: center;"><?= htmlspecialchars($jadwal['NOMOR_KEGIATAN']) ?></td>
                          <td style="text-align: center;"><?= htmlspecialchars($jadwal['JP_HARI']) ?></td>
                        </tr>
                      <?php } ?>
                    </tbody>
                  </table>
